...
/// Windows desktop implementation of [AttriaxPlatform].
///
/// Under the native-engine re-wrap, Flutter-Windows stops running the in-Dart
/// engine and instead drives the shared Kotlin Multiplatform core — Attriax's
/// reference engine — through its C-ABI shared library (`attriax_core.dll`) over
/// `dart:ffi`. Every command forwards to [AttriaxCoreBindings.dispatch] as a
/// `{"ok":…}` JSON envelope, mirroring the JNI (Android) and JS (web) bindings
...
class AttriaxWindows extends AttriaxPlatform {
  /// [bindings] is an injection seam for tests; production callers omit it and
  /// the plugin loads the bundled `attriax_core.dll` on [initialize].
  AttriaxWindows({AttriaxCoreBindings? bindings})
    : _injectedBindings = bindings;

  /// Registers this class as the Windows implementation of `attriax_flutter`.
  static void registerWith() {
    AttriaxPlatform.instance = AttriaxWindows();
  }

  static const String _logName = 'attriax.windows';
  ...
  Future<AttriaxCreateDynamicLinkResult> createDynamicLink({
    ...
    throw StateError(
      'createDynamicLink is not supported by the Attriax Windows engine.',
    );
  }
  ...
  Object? _dispatch(String method, [Map<String, Object?>? args]) {
    ...
    final handle = _handle;
    if (bindings == null || handle == nullptr) {
      throw StateError('Attriax Windows engine is not initialized.');
    }
  ...
  void _log(String method, Object error, StackTrace stackTrace) {
    developer.log(
      'AttriaxWindows.$method failed: ${error.runtimeType}',
      name: _logName,
      error: error,
      stackTrace: stackTrace,
    );
