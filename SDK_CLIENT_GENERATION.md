...
The generated Flutter transport layer lives in `flutter-plugin/attriax_sdk_client/`.
...
Treat `attriax_sdk_client` as generated output, not handwritten source.
...
2. Deletes the previous `flutter-plugin/attriax_sdk_client/` output.
...
5. Runs `flutter pub get` in `flutter-plugin/`.
6. Runs `dart run build_runner build --delete-conflicting-outputs` in `attriax_sdk_client/`.
...
1. `flutter pub get` in `flutter-plugin/`
2. `dart analyze attriax attriax_sdk_client`
3. `flutter test` in `flutter-plugin/attriax`
...
- `flutter-plugin/attriax_sdk_client/lib/`
- `flutter-plugin/attriax_sdk_client/doc/`
- `flutter-plugin/attriax_sdk_client/.openapi-generator/`
...
- `flutter-plugin/attriax_sdk_client/pubspec.yaml`
- `flutter-plugin/attriax_sdk_client/analysis_options.yaml`
- `flutter-plugin/attriax_sdk_client/README.md`

## Why It Is A Separate Package

The current supported layout keeps the generated client in `attriax_sdk_client/` instead of embedding it directly into `attriax/lib/`.
...
## Can It Be Embedded Into `attriax`?

Yes, technically.

The same workflow can target `flutter-plugin/attriax/lib/src/generated/` instead of `flutter-plugin/attriax_sdk_client/`.
...
For the current repository, the more reliable option is to keep generation isolated in `attriax_sdk_client/` and have `attriax/` consume it internally.
